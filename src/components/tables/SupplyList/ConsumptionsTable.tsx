...
// Interface para os filtros (mesma do pai)
...
interface ConsumptionsTableProps {
  ...
  refreshKey?: number; // quando muda, força reload
...
export default function ConsumptionsTable({
    ...
    const loadTransactions = async () => {
      try {
        ...
        const url = queryString
          ? `${API_ENDPOINTS.transaction}/list?${queryString}`
          : `${API_ENDPOINTS.transaction}/list`;

        const data = await authFetch<TransactionResponse[]>(url);
  ...
  }, [filters, refreshKey]); // recarrega quando filters ou refreshKey mudam
  ...
  if (transactions.length === 0) {
    return (
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
        <div className="p-8 text-center text-gray-500 text-theme-sm">
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/5">
            {transactions.map((tx) => (
              ...
              >
                ...
                <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm">
                  {formatNumber(tx.price_unit, 2)}
                </TableCell>

                <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm">
                  {formatNumber(tx.total_price, 2)}
                </TableCell>
