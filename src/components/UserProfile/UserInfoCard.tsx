...
import TwoFactorAuthSettings from "./TwoFactorAuthSettings"; // Importe o novo componente
...
interface UserInfoCardProps { // Renomeado de UserMetaCardProps para UserInfoCardProps
  userId: string;
}
...
export default function UserInfoCard({ userId }: UserInfoCardProps) {
  ...
  const [user, setUser] = useState<UserDetail | null>(null); // Pode ser null inicialmente

  // Estado para 2FA, que será passado para TwoFactorAuthSettings
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        if (userData) {
          setUser(userData);
          setTwoFactorEnabled(userData.two_factor_enabled); // Atualiza o estado do 2FA
        }
  ...
  // Callback para atualizar o estado do 2FA no UserInfoCard quando o TwoFactorAuthSettings o altera
  const handleTwoFactorUpdate = (newStatus: boolean) => {
    setTwoFactorEnabled(newStatus);
    // Opcional: Se o user object também precisa ser atualizado, você pode fazer uma nova busca
    // ou atualizar o estado do user diretamente aqui.
    setUser(prevUser => prevUser ? { ...prevUser, two_factor_enabled: newStatus } : null);
  };
  ...
  return (
      <div className="rounded-2xl border border-gray-200 p-5 dark:border-gray-800 lg:p-6">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          ...
          {/* Botões para abrir modais de preferências e 2FA */}
          <div className="flex flex-col gap-3 lg:items-end">
            ...
            {/* Adicione o componente TwoFactorAuthSettings aqui */}
